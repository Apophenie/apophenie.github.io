 ...
 *
 * ★ Douze opérateurs lisent la cible et ont le DROIT de se retirer quand leur
...
test('★ retraits — sur la cible sous-jacente d’un mot, douze opérateurs s’en vont', () => {
  const rangs = cibleDeValeurs([26, 5, 18, 7]);
  const retires = operateursRetires(catalogue, rangs);
  // Dix, plus les deux redécoupages qui accolent (`mrdf`, `mrfE`), qui lisent
  // la cible comme leurs modèles. `megf` n'est pas compté : inactif en
  // recherche, il n'a rien à retirer.
  assert.equal(retires.length, 12, 'toute la famille qui lit la cible');
