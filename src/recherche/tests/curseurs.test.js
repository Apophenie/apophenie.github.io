...
import {
  ...
  BUDGET_TOTAL_MS, BUDGET_MS_FILET,
} from '../../config.js';
...
test('fouille — elle ne change rien là où le budget ne mordait pas', () => {
  // Mesure consignée dans `config.js` : sur les saisies que la recherche
  // termine, monter la réglette ne peut rien ajouter — il n'y avait rien à
  // ajouter. C'est ce qui rend la réglette honnête plutôt que magique.
  for (const s of ['hope', 'https://hope-hope-hope.fr/']) {
    const base = moteur.enumerer(s, { fouille: 0 });
    assert.equal(base.tronque, false, `« ${s} » n’était pas censée être tronquée`);
...
test('fouille — elle finit la recherche là où le budget mordait', () => {
  // L'autre moitié de la mesure : sur la seule saisie tronquée du relevé, un
  // seul cran de plus suffit à terminer la recherche et la tête double.
  const s = 'La numérologie est une science exacte, disent-ils';
  const court = moteur.enumerer(s, { fouille: 0 });
  const long = moteur.enumerer(s, { fouille: 1 });
  assert.equal(court.tronque, true, 'cette saisie doit épuiser le budget au cran 0');
  assert.equal(long.tronque, false, 'un cran de plus doit suffire à la terminer');
